import { Icon, Input, Table, Button, DatePicker, Modal } from 'antd';
...
export class PayrollToApproveTable extends Component {
  state = {
    ...this.state,
    searchText: '',
    searchedColumn: '',
  ...
  componentDidMount() {
    // console.log(`PayrollToApproveTable`, this.props);
  }
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <Link
              to={{
                pathname: `/payslip/${record.id}`,
                state: { payrollDetailsData: record, to_approve: true, parent_path: 'payslip_to_approve' },
              }}
            >
              {text}
            </Link>
...
const mapStateToProps = state => {
  // console.log(`In PayrollToApproveTable state`, state);
  return {
    payrollList: state.payroll_to_approve.payrolls,
  };
};
...
export default connect(mapStateToProps, mapDispatchToProps)(PayrollToApproveTable);

...
import { DELETE_CUSTOMER_SUCCESS, EDIT_CUSTOMER_SUCCESS, FETCH_CUSTOMER_SUCCESS, HANDLE_CUSTOMER_MODAL_CANCEL, HANDLE_CUSTOMER_MODAL_SHOW, INITIATE_CUSTOMER_FAILURE, INITIATE_CUSTOMER_REQUEST, POST_CUSTOMER_SUCCESS, SET_CUSTOMER_MODAL_ITEM } from "./customer.constants";
...
export const editCustomerSuccess = (customer) => {
    ...
    return {
        type: EDIT_CUSTOMER_SUCCESS || 'EDIT_SALE_SUCCESS',
        payload: customer,
    };
};
...
export const postCustomer = (customer) => {
  let _name = computeCustomerName()
    let params = {
      "name": _name,
      "employee_id": customer.worker,
      "salary_id": customer.salary
  }
  return (dispatch) => {
      dispatch(initiateCustomerRequest);
      let stored_token = localStorage.getItem('token')

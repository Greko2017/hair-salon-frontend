...
import { postServiceLine } from '../serviceline.actions';
const { Option } = Select;
...
export class ServiceLineModalInner extends Component {
  ...
  render() {
    const {form, modalServiceLineVisible, customers, service_names, isOnCreate, services, item, service_by_id } = this.props
      ...
      >
        <Form {...formItemLayout} className="serviceline-form">
          <Form.Item label="Parent" className="serviceline_modal__content">
            {getFieldDecorator('parent', { initialValue: item !== undefined && isOnCreate === false ? item.service_by_id.id : service_by_id !== undefined ? service_by_id.id : null })(
                <Select disabled
                    ...
                    {services.map(service => <Option disabled key={service.id} value={service.id}>{service.name}</Option>)}
                </Select>

                )}
          </Form.Item>
          <Form.Item label="Customer" className="serviceline_modal__content">
            {getFieldDecorator('customer_id', { initialValue: item !== undefined && isOnCreate === false ? item.customer_id : null })(
                <Select
                    showSearch={true}
                    ...
                    {customers.map(customer => <Option key={customer.id} value={customer.id}>{customer.firstname} {customer.lastname !== null ? customer.lastname : ''}</Option>)}
                </Select>

                )}
          </Form.Item>
          <Form.Item label="Service" className="serviceline_modal__content">
            {getFieldDecorator('lookup',  { initialValue: item !== undefined && isOnCreate === false ? item.lookup.id : null })(
                <Select
                    showSearch={true}
                    placeholder="Service"
                    optionFilterProp="children"
          ...
          <Form.Item label="Amount" {...amountFormItemLayout} style={{marginBottom:".19rem"}}>
            ...
            <Form.Item
              style={{ display: 'inline-block', width: '18%', marginLeft: "0.15rem"}}>
          ...
          <Form.Item {...isCreditFormItemLayout}>
            {getFieldDecorator('is_credit', { initialValue: item !== undefined ? item.is_credit : true,

        ...
        <Form {...formItemLayout} className="service-form">
          <Form.Item label="Worker" className="service_modal__content">
            {getFieldDecorator('worker', {
              initialValue: item !== undefined && isOnCreate === false ? item.employee_id : null,
            })(
              <Select
              ...
              >
                {employeelist.map(salary => (
                  <Option key={salary.id} value={salary.id}>{`${salary.gender === 'male' ? 'Mr.' : 'Mm'} ${
...
const mapDispatchToProps = dispatch => {
  return {
    handleServiceModalCancelAction: () => {
      dispatch(handleServiceModalCancelAction());
    },
    postService: new_service => {
      dispatch(postService(new_service));
    },
    editService: service => {
...
export default compose(withConnect, memo)(ServiceModal);

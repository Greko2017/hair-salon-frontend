        ...
        <Form {...formItemLayout} className="service-form">
          <Form.Item label="Saler" className="service_modal__content">
            {getFieldDecorator('saler', {
              initialValue: item !== undefined && isOnCreate === false ? item.saler_id : null,
            })(
              <Select
                showSearch={true}
                placeholder="Saler"
              ...
              >
                {customerList.map(customer => (
                  <Option key={customer.id} value={customer.id}>
                    {' '}
                    {`${customer.gender === 'male' ? 'Mr.' : 'Mm'} ${customer.firstname}`}{' '}
                  </Option>
                ))}
              </Select>,
            )}
...
const mapStateToProps = state => {
  console.log(`-- In SaleModal state:>>`, state);
  return {
...
const mapDispatchToProps = dispatch => {
  return {
    handleSaleModalCancelAction: () => {
      dispatch(handleSaleModalCancelAction());
    },
    postSale: new_sale => {
      dispatch(postSale(new_sale));
    },
    editSale: sale => {
...
export default compose(withConnect, memo)(SaleModal);
